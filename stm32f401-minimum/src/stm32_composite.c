/****************************************************************************
 * boards/arm/stm32/stm32f401-minimum/src/stm32_composite.c
 *
                                ...
                                struct usbdevclass_driver_s **classdev)
{
  ...
  uinfo("Bind LUN=0 to /dev/smart0\n");
  ret = usbmsc_bindlun(g_mschandle, "/dev/smart0", 0, 0, 0, false);
  if (ret < 0)
    {
      uerr("ERROR: usbmsc_bindlun failed for LUN 1 at /dev/smart0: %d\n",
           ret);
